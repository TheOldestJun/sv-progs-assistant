    ...
    <div className="overflow-hidden rounded-xl border border-border">
      <table className="w-full text-sm">
        <thead className="bg-surface-secondary">
          <tr>
            <th className="px-4 py-3 text-left font-medium text-text-secondary">{tReset("user")}</th>
            <th className="px-4 py-3 text-left font-medium text-text-secondary">{t("email")}</th>
            <th className="px-4 py-3 text-left font-medium text-text-secondary">{tReset("requested")}</th>
            <th className="px-4 py-3 text-right font-medium text-text-secondary">{t("actions")}</th>
          </tr>
        </thead>
          ...
          {requests.map((req) => (
            <tr key={req.id} className="hover:bg-surface">
              ...
              <td className="px-4 py-3 text-right">
                ...
                >
                  {isPending ? tReset("resetting") : tReset("reset")}
                </button>
              </td>
            </tr>
          ))}
