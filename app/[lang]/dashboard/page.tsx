...
const ARCHIVE_TAB = { role: "__archive__", labelKey: "archive", icon: "🗃️" };
...
export default async function DashboardPage({
  ...
  if (!session) redirect({ href: "/login", locale: lang });
      ...
      <div className="mb-8 flex items-center justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">
            {name || email} — {roles.map((r) => tRoles(ROLE_META[r]?.labelKey || r)).join(", ")}
          </p>
        </div>
      ...
      {dashboards.length === 0 ? (
        ...
        (() => {
          const tabs: { role: string; label: string; icon: string }[] = dashboards.map((r) => ({
            role: r,
            label: tRoles(ROLE_META[r]?.labelKey || r),
          ...
          if (!isAdmin && !roles.includes("REQUESTER")) {
            tabs.push({ ...ARCHIVE_TAB, label: t("tabs.archive") });
            components.push(
