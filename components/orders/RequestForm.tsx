/*
 * RequestForm — единая форма создания заявки на снабжение.
 * Переиспользуется тремя дашбордами: Requester (заказчик), Warehouse (склад),
 * HeadOfSupply (начальник снабжения) — у всех одинаковый функционал:
 *  - до MAX_ITEMS позиций (лимит шаблона REQUEST.xlsx) со счётчиком
 *  - Enter в количестве → фокус на «ТМЦ» следующего пункта (новый создаётся при необходимости)
 *  - диалог подтверждения при отправке («Отправить заявку»)
 *  - создание продуктов/единиц/заявителей через Autocomplete (оптимистичные ID)
 *
 * Отличия дашбордов включаются пропсами:
 *  - showRequester — выбор заявителя (у заказчика его нет: заявитель определяется по сессии)
 *  - excelButton  — кнопка «Сохранить в Excel» (только у заказчика)
 */
"use client";
...
import { MAX_ITEMS } from "@/components/orders/requestExcel";
...
interface RequestFormProps {
  /** Показывать выбор заявителя (false — заказчик, заявитель определяется по сессии) */
  showRequester?: boolean;
  /** Показывать кнопку «Сохранить в Excel» */
  excelButton?: boolean;
}

export function RequestForm({ showRequester = true, excelButton = false }: RequestFormProps) {
  ...
  // Сигнал фокуса для программного перехода на поле «ТМЦ» следующего пункта (Enter в количестве)
  const [focusSignal, setFocusSignal] = useState<{ itemId: string; tick: number } | null>(null);
  ...
  function addItem() {
    setItems((prev) => (prev.length < MAX_ITEMS ? [...prev, createEmptyItem()] : prev));
  }

  /*
   * Enter в поле количества заполненного пункта — переход к заполнению следующего:
   * фокусируем уже существующую строку или добавляем новую и фокусируем её «ТМЦ».
   */
  function handleItemKeyDown(e: React.KeyboardEvent, item: OrderItem) {
    ...
    if (next) {
      setFocusSignal((prev) => ({ itemId: next.id, tick: (prev?.tick ?? 0) + 1 }));
      return;
    }
    if (items.length >= MAX_ITEMS) return;
    const newItem = createEmptyItem();
    setItems((prev) => [...prev, newItem]);
    setFocusSignal((prev) => ({ itemId: newItem.id, tick: (prev?.tick ?? 0) + 1 }));
  }
  ...
  function handleRequesterCreate(name: string): AutocompleteItem {
    const optimisticId = `optimistic-${Date.now()}`;
    ...
    return { id: optimisticId, title: name };
  }
  ...
  function submitOrder() {
    ...
    const payload = {
      ...(showRequester ? { requesterId: requester!.id } : {}),
      created: date,
    ...
    createOrder.mutate(payload, {
      onSuccess: () => {
        ...
        if (showRequester) setRequester(null);
      },
      onError: (err) => {
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-4">
          ...
          <div key={item.id} className="space-y-2">
            <div className="items-end gap-3 sm:grid sm:grid-cols-[3fr_80px_100px_32px] max-sm:space-y-3">
              <Autocomplete
                ...
                focusKey={focusSignal?.itemId === item.id ? focusSignal.tick : null}
              />
        ...
        {!canAdd && items.length >= MAX_ITEMS && (
          <span className="text-xs text-text-secondary">{items.length} / {MAX_ITEMS} позиций</span>
        )}
      </div>
