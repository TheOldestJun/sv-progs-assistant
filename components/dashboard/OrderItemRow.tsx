...
interface OrderItemRowProps {
  ...
  onAskRequester?: () => void;
  /** Режим выбора позиций для создания пропуска: доступна ли КОНКРЕТНАЯ позиция (по её статусу) */
  selectable?: boolean;
  /** Подсказка (tooltip) на невыбираемых позициях, почему чекбокс отключён */
        ...
        <td className="px-2 py-1.5 sm:px-4 sm:py-0.5 max-sm:flex max-sm:items-center max-sm:gap-2 max-sm:p-0 sm:flex sm:items-start sm:gap-2">
          {onSelectChange && (
            <label
              ...
              title={!selectable && selectionHint ? selectionHint : ""}
              className={`mt-0.5 flex size-4 shrink-0 items-center justify-center ${selectable ? "cursor-pointer" : ""}`}
            >
              <input
                type="checkbox"
