...
-- 2. Создаём таблицу OrderConfirmToken (per-item token, связан с OrderItem)
CREATE TABLE `OrderConfirmToken` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `OrderConfirmToken_orderItemId_key` (`orderItemId`),
  UNIQUE INDEX `OrderConfirmToken_token_key` (`token`),
  INDEX `OrderConfirmToken_orderItemId_idx` (`orderItemId`),
  INDEX `OrderConfirmToken_token_idx` (`token`),
  CONSTRAINT `OrderConfirmToken_orderItemId_fkey` FOREIGN KEY (`orderItemId`) REFERENCES `OrderItem`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
